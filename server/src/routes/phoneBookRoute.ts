...
phonebookRoutes.post('/add', async (req:Request, res:Response) => {
    ...
    const alreadyExistName=await Phonebook.find({first_name:first_name,last_name:last_name})
...
const {first_name,last_name,phone_number}=req.body
    const alreadyExistName=await Phonebook.find({$and:[{first_name:first_name},{last_name:last_name},{_id:{$ne:req.params.id}}]})
    if(alreadyExistName.length>0){
      res.json('this contact name already added')
      return 0
    }
    const alreadyExistNum=await Phonebook.find({$and:[{phone_number:phone_number},{_id:{$ne:req.params.id}}]})
    if(alreadyExistNum.length>0){
      res.json('this phone number already added')
      return 0
    }
